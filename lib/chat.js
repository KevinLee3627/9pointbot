...
async function initChat() {
	...
	const chatClient = new ChatClient(await twitchRefreshableAuthProvider(), { channels: [config.broadcaster.name] });
	...
	chatClient.onRegister( () => {
		chatClient.say(config.broadcaster.name, '9pointbot is fully initialized! peepoArrive');
	})
	...
	chatClient.onMessage(async (channel, username, msg, fullMsg) => {
		...
		if (user.isFollowing && !receivedPassivePoints) {
			...
			await mongo.updateUser(username, 'points', newPointsValue);
